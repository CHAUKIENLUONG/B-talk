...
function updateTable() {
    ...
    $('#selectedCombos').val(selectedCombos.length > 0 ? selectedCombos.join(',') : null);
...
function updateSeatImage(seatElement, state) {
    const imgElement = seatElement.querySelector('img');
    
    switch(state) {
        case 'chosen':
            imgElement.src = '/images/seat/seat_ischosen.png';
            break;
        case 'booked':
            imgElement.src = '/images/seat/seat_isbooked.png';
            break;
        default:
            imgElement.src = '/images/seat/seat_empty.png';
    }
}
    ...
    seat.addEventListener('click', function() {
        if (!this.classList.contains('booked')) {
            if (this.classList.contains('chosen')) {
                ...
                updateSeatImage(this, 'empty');
            } else {
                this.classList.add('chosen');
                updateSeatImage(this, 'chosen');
            }
        }
    });
});

// Cập nhật ghế đã được đặt khi tải trang
function initializeBookedSeats(bookedSeats) {
    bookedSeats.forEach(seatId => {
        const seatElement = document.querySelector(`[data-seat-id="${seatId}"]`);
        if (seatElement) {
            seatElement.classList.add('booked');
            updateSeatImage(seatElement, 'booked');
        }
    });
}