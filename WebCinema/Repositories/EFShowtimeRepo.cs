...
namespace WebCinema.Repositories
{
    public class EFShowtimeRepo : IShowtimeRepo
    {
        ...
        public async Task<bool> IsShowtimeDuplicateAsync(int movieId, string roomId, DateTime showDate, int screenTimeId)
        {
            return await _context.Showtimes.AnyAsync(s =>
                s.MovieId == movieId &&
                s.RoomId == roomId &&
                s.ShowtimeDate.Date == showDate.Date && // Kiểm tra cùng ngày
                s.ScreenTimeId == screenTimeId);    // Kiểm tra cùng khung giờ
        }
